...
DIGITECHNEWS_INFO="Welcome to the DigiTechNews Official Sentora Installer"
...
$PACKAGE_INSTALLER wget nano htop varnish postfix proftpd
...
# Downloading Sentora Templates
echo -e "\n Downloading... Sentora Templates"
cd /etc/sentora/panel/etc/styles/
wget https://github.com/auxio/Zentora/archive/master.zip
unzip master.zip
rm master.zip
mv Zentora-master Zentora
chmod 777 -R /etc/sentora/panel/etc/styles/Zentora
echo -e "\n-- Zentora Template Installed --"

echo -r "\n Downloading... phpMyAdmin 4.8.1"
...
mv phpmyadmin phpmyadmin-bkp
mv phpMyAdmin-4.8.1-all-languages phpmyadmin
cp /etc/sentora/panel/etc/apps/phpmyadmin-bkp/config.inc.php /etc/sentora/panel/etc/apps/phpmyadmin/config.inc.php
